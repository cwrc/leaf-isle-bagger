...
def parse_args():
    ...
    parser.add_argument(
        "--output", required=True, help="Location to store JSON (like) output file."
    )
    parser.add_argument(
...
def process(args, session, output_file):
    ...
    logging.info(f"Audit: Drupal nodes before media inclusion - length [{len(node_list)}] - details {node_list}")
    ...
    logging.info(f"Audit: Drupal nodes with media changes - length [{len(node_list)}] - details {{node_list}")
